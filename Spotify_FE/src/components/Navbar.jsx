...
const ShazamModal = ({ setShowShazamModal, setShazamLoading, visible }) => {
    ...
    const identifySong = async () => {
        try {
            ...
            const filename = audioSource === 'recording' ? 'recording.webm' : 'upload.' + audioBlob.name?.split('.').pop() || 'mp3';
            
            // Log what we're sending
            console.log("Sending audio for identification:", {
                type: audioBlob.type,
                size: audioBlob.size,
                filename
            });
            ...
            // Process matches
            const songMatches = response.data.matches.map(match => ({
                ...match,
            ...
            // If there's a message in the response, show it as an error/notice
