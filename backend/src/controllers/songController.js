...
const findMatches = async (req, res) => {
    ...
    try {
        ...
        //     setTimeout(() => reject(new Error('Audio processing timed out')), 60000)
        // );
        
        // Add response tracking flag
            ...
            try {
                ...
                if (req.file.mimetype.includes('webm')) {
                    try {
                        ...
                        samples = wavDecoded.channelData[0];
                ...
                } else if (fileExtension === 'wav' || req.file.mimetype.includes('wav')) {
                    try {
                    ...
                    try {
                        ...
                        samples = decodedAudio.getChannelData(0);
                    } catch (decodeError) {
                        console.error("Error decoding audio:", decodeError);
                    }
                }
                
                // If we couldn't get samples, return fallback
                if (!samples) {
                    if (!hasResponded) {
                        ...
                        const fallbackMatches = songs.map(song => ({
                            ...
                            album: song.album,
                            image: song.image,
                            confidence: 10 // 10% confidence for fallbacks
                        }));
                ...
                fingerprints = generateFingerprint(samples);
                console.log(`Generated ${fingerprints.length} fingerprints`);
                
                // Find songs with fingerprints
                ...
                for (let i = 0; i < songs.length; i += BATCH_SIZE) {
                    ...
                    const batchResults = await Promise.all(batch.map(async (song) => {
                        if (!song.fingerprints?.length) return null;
                    ...
                    matches.push(...batchResults.filter(match => match !== null));
                }
                ...
                if (!hasResponded) {
                    hasResponded = true;
                    console.log("Sending successful response with matches:", 
                ...
                if (!hasResponded) {
                    hasResponded = true;
                    console.log("Sending error response:", error.message || 'Error processing audio');
                    return res.json({ 
                        success: false, 
        ...
        // Remove the Promise.race with timeout
        await processAudio();
